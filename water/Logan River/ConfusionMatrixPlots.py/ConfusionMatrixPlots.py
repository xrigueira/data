...
# Set colors and labels for bars, columns, and rows of each panel
#########################################
...
# Numbers are the results of an initial model run
#########################################
...
fig, axes = plt.subplots(nrows=6, ncols=4, figsize=(16, 10))
for i, ax in enumerate(axes.flat):
    p1 = ax.bar(1, np.array(TP_aggregate).flatten()[i], color=colors[0])
    p2 = ax.bar(1, np.array(FN_aggregate).flatten()[i], color=colors[1], bottom=np.array(TP_aggregate).flatten()[i])
    p3 = ax.bar(1, np.array(FP_aggregate).flatten()[i], color=colors[2], bottom=np.array(FN_aggregate).flatten()[i]+np.array(TP_aggregate).flatten()[i])
    p4 = ax.axhline(y=TP_rules[i], linewidth=1.5, linestyle='--', color='k')
    p5 = ax.axhline(y=np.array(FN_aggregate).flatten()[i] + np.array(TP_aggregate).flatten()[i] + FP_rules[i], linewidth=1.5, linestyle='--', color='gray')
    # ax.legend((p1[0],p2[0],p3[0]),('True Positives', 'False Negatives', 'False Positives'))
for ax, col in zip(axes[0], cols):
    ax.set_title(col)
for ax, row in zip(axes[:, 0], rows):
